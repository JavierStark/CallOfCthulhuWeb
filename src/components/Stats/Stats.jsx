...
const Stats = () => {
  const [stats, setStats] = useState({
    str: 0, con: 0, dex: 0, app: 0, pow: 0, siz: 0, int: 0, edu: 0, luck: 0, sanity: 0, hp: 0, db: 0, build: 0, mov: 0
  });
  const navigate = useNavigate();

  const roll = (n, dice) => {
    let result = 0;
    for (let i = 0; i < n; i++) result += Math.floor(Math.random() * dice + 1);
    return result;
  }
  ...
  const rollStats = () => {
    ...
    for (let k in newStats) {
      ...
      if (['siz', 'int', 'edu', 'luck'].includes(k)) newStats[k] = roll(2, 6) + 6; else newStats[k] = roll(3, 6);
  ...
  const updateOtherStats = () => {
    setStats(pS => {
      ...
      return {
        ...pS, sanity: pS.pow, hp: Math.floor((pS.con + pS.siz) / 10), db: db, build: build, mov: getMov()
      }
    })
  }
  ...
  return (<form onSubmit={handleSubmit} className={'stats-form'}>
    <div className={'stats-container'}>
      <div className={'primary-stats stats'}>
        <div>
          <label>Strength</label>
          <input className={'stats-input'} type={'number'}
                 name={'str'}
                 value={stats['str'] || 0}
      ...
      <div className={'other-stats stats'}>
        ...
        <div>
          ...
          <input className={'stats-input'} type={'text'}
                 name={'db'}
                 value={stats['db'] || 0}
                 disabled/>
        </div>
      ...
      <button type={'submit'} className={'submit-stats-button'} name={'submitStats'}>Submit
      </button>
    </div>
  </form>);
};
